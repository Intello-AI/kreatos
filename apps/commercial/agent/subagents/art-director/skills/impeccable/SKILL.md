---
name: impeccable
description: "Método senior de craft de UI para el art-director kreatos (razonamiento, no código). Brief/producto/marca, jerarquía, contraste, restraint, crítica anti-slop. Cárgala para armar el brochure/brief del producto desde el lead+marca y razonar el concepto. El contrato del template kreatos manda sobre su stack/scripts."
---

> **ADAPTACIÓN KREATOS (léela antes que el resto).** Eres el **art-director**:
> compones el SPEC de diseño, NO escribes código y NO tienes sandbox. Por eso:
> - **Ignora el `## Setup` que corre `node scripts/*.mjs`** (context.mjs,
>   palette.mjs, detect.mjs, live) — no hay sandbox ni navegador aquí. Tu
>   "contexto de producto" (PRODUCT.md) es el **brief + lead + ficha de marca**
>   que ya trae `get_site_brief`; tu paleta arranca de los **colores de marca**
>   del lead, no de palette.mjs.
> - Usa esta skill como **razonamiento de diseño** (jerarquía, contraste,
>   restraint, brief/producto/brand, anti-slop, crítica) para pensar mejor el
>   SPEC — NO como generador de código ni de comandos `$impeccable`.
> - **El contrato del template kreatos MANDA sobre cualquier consejo de aquí**:
>   stack fijo (Next 16 + Tailwind v4 + shadcn), cero dependencias nuevas (nada
>   de GSAP), motion solo con los primitives del motor, colores solo tokens
>   semánticos, copy vía next-intl, y las secciones que el motor no soporta van
>   como `custom` (ver skill `section-patterns`). Si un consejo de impeccable
>   choca con eso, gana el template.
> - Las `reference/*.md` útiles para ti: `init.md`/`product.md`/`brand.md`
>   (arman el brief/brochure del producto desde el lead+marca), `shape.md`,
>   `distill.md`, `craft.md`, `layout.md`, `typeset.md`, `colorize.md`. Las de
>   iteración en navegador (`live.md`, `animate.md`, `optimize.md`) no aplican.
