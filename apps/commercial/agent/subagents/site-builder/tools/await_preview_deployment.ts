  ...
  async execute({ siteId, versionN, commitSha }) {
    ...
    while (Date.now() < deadline) {
      ...
      if (deployment?.state === "READY" && deployment.url) {
        await setVersionPreview({ siteId, versionN, previewUrl: deployment.url })
        await setSiteStatus(siteId, "preview")
        await addActivity({
          leadId: site.lead_id,
          type: "site_preview_ready",
          note: `v${versionN} lista para revisión: ${deployment.url}`,
          actor: "site-builder",
        })
        return { state: "READY" as const, previewUrl: deployment.url }
      }
